...
def login(driver, user, password):
    ...
    driver.find_element_by_css_selector(
        "input[id='user-name']").send_keys(user)
    driver.find_element_by_css_selector(
...
def add_cart(driver, n):
    ...
    for i in range(n):
        ...
        driver.find_element_by_css_selector(
            "button.inventory_details_back_button").click()
        acum += 1
    print(timestamp() + ' ' + str(acum) +
          ' items were added to the shopping cart.')
...
def remove_cart(driver, n):
    ...
    for i in range(n):
        ...
        acum += 1
    print(timestamp() + ' ' + str(acum) +
          ' items were removed from the shopping cart.')
